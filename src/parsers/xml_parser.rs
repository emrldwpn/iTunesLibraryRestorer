        ...
        match reader.read_event_into(&mut buffer) {
            ...
            Ok(Event::End(_)) => {
                if song_found && !get_path(&path).contains(SONG_PATH) {
                    ...
                    if song.is_some() {
                        let some_song: SongInfo = song.unwrap();
                        let artist: String = String::from(some_song.artist());
                        let song_map: &mut HashMap<String, SongInfo> = artist_map.entry(artist.to_string()).or_insert(HashMap::new());
